...
La reconnaissance d’entités nommées (souvent abrégé en NER d’après l’anglais <i>Named Entity Recognition</i>) est une tâche de NLP consistant à apporter étiqueter les séquences de mots d’un texte qui sont des noms de choses (personnes, sociétés, gènes, protéines, etc.).<br>
...
<p style="text-align:justify;">
Le jeu de données le plus connu et faisant référence en NER, est le jeu de données <a href="https://www.clips.uantwerpen.be/conll2003/ner/"> CoNLL-2003 (<i>Conference on Computational Natural Language Learning</i>)</a> de Erik F. Tjong Kim Sang et Fien De Meulderet (2003). Créé pour l’anglais et l’allemend, les autres langues ont généralement adopté son formatage. <br>
Ci-dessous un exemple d’une ligne de ce jeu de données : <br>
</p>
<center>
<figure class="image">
...
-	<a href="http://dx.doi.org/10.1016/j.artint.2012.03.006">WikiNER</a> de Nothman et al. (2013) qui est un jeu de données (pour la partie en français) de 120 682 données d’entraînement et 13 410 de test. Les entités annoncées sont LOC (Localisation), ORG (Organisations), PER (Personnes) et MISC (Divers) et sont réparties de la façon suivante :<br>
<table>
<thead>
<tr>
<th>Split</th>
...
-	<a href="https://arxiv.org/abs/1902.00193">Wikiann</a> by Rahimi et al. (2019) based on Pan, Xiaoman, et al. (2019) qui est un jeu de données avec 20 000 données d’entraînement, 10 000 de validation et 10 000 de test. Les entités annoncées sont LOC, ORG, PER et MISC et sont réparties de la façon suivante :<br>
...
<br><br>
Il s’agit ici des principaux jeux de données « propres », au sens qu’ils sont utilisables facilement car disponibles sur le Hub de Hugging Face, mais nous verrons dans la section suivante que la qualité des données de certains jeux de données cités à l’instant est questionnable et a donc nécessité un nettoyage avant de pouvoir les utiliser pour entraîner un modèle. 
<br><br>


A noter qu’il existe d’autres jeux de données de NER en français mais étant soit de moins bonne qualité, soit nécessitent énormément de prétraitement pour être exploitable, ou soit trop spécialisé. On peut par exemple citer les jeux de données suivants :
...
-	<a href="https://arxiv.org/abs/1703.00948.pdf">DAWT</a> de Spasojevic et al. (2017) n’est plus disponible en ligne suite au rachat de l’entreprise ayant créé ce jeu de données.<br>


Dans le cadre de nos expérimentations, nous avons décidé de travailler dans le cadre de deux configurations. 
...
A noter également qu’à l’issue du nettoyage individuel, un nettoyage supplémentaire doit être effectué au niveau collectif. En effet, une donnée présente dans l’échantillon d’entraînement d’un jeu de données A et donc non présente dans l’échantillon de test de A, peut être présente dans l’échantillon de test de B, ce qui crée une fuite lors de la création du jeu de données A+B.
...
-	Multiconer :<br>
o	Fuites entre l'échantillon d'entraînement et l'échantillon de test : 13 lignes soit 0,083 %<br>
o	Fuites entre l'échantillon de validation et l'échantillon de test : 0 lignes soit 0 %<br>
o	Lignes dupliquées dans l'échantillon d'entraînement : 170 lignes soit 1,081 %<br>
o	Lignes dupliquées dans l'échantillon de validation : 2 lignes soit 0,242 %<br>
o	Lignes dupliquées dans l'échantillon de test : 2 lignes soit 0,233 %<br>
Ainsi entre les fuites et les duplications, les performances mesurées sur l’échantillon de test sont faussées d’au moins 1,750%.<br>

-	Multinerd :<br>
o	Fuites entre l'échantillon d'entraînement et l'échantillon de test : 69 lignes soit 0,049 %<br>
o	Fuites entre l'échantillon de validation et l'échantillon de test : 20 lignes soit 0,114 %<br>
o	Lignes dupliquées dans l'échantillon d'entraînement : 2600 lignes soit 1,846 %<br>
o	Lignes dupliquées dans l'échantillon de validation : 58 lignes soit 0,329 %<br>
o	Lignes dupliquées dans l'échantillon de test : 58 lignes soit 0,328 %<br>
Ainsi entre les fuites et les duplications, les performances mesurées sur l’échantillon de test sont faussées d’au moins 0,831%.<br>

-	Pii-masking-200k :<br>
	Ce jeu de données ne contient ni fuites ni duplications de données.<br>

-	Wikiner :<br>
o	Fuites entre l'échantillon d'entraînement et l'échantillon de test : 22 lignes soit 0,019 %<br>
o	Fuites entre l'échantillon de validation et l'échantillon de test : 20 lignes soit 0,331 %<br>
o	Lignes dupliquées dans l'échantillon d'entraînement : 562 lignes soit 0,49 %<br>
o	Lignes dupliquées dans l'échantillon de validation : 17 lignes soit 0,282 %<br>
o	Lignes dupliquées dans l'échantillon de test : 17 lignes soit 0,127 %<br>
...
-	Wikiann :<br>
o	Fuites entre l'échantillon d'entraînement et l'échantillon de test : 742 lignes soit 3,71 %<br>
o	Fuites entre l'échantillon de validation et l'échantillon de test : 473 lignes soit 4,73 %<br>
o	Lignes dupliquées dans l'échantillon d'entraînement : 1889 lignes soit 9,445 %<br>
o	Lignes dupliquées dans l'échantillon de validation : 644 lignes soit 6,44 %<br>
o	Lignes dupliquées dans l'échantillon de test : 644 lignes soit 6,44 %<br>
...
o	Fuites entre l'échantillon d'entraînement et l'échantillon de test : 1071 lignes soit 0,371 %<br>
o	Fuites entre l'échantillon de validation et l'échantillon de test : 613 lignes soit 1,799 %<br>
o	Lignes dupliquées dans l'échantillon d'entraînement : 1839 lignes soit 0,638 %<br>
o	Lignes dupliquées dans l'échantillon de validation : 8 lignes soit 0,023 %<br>
o	Lignes dupliquées dans l'échantillon de test : 8 lignes soit 0,019 %<br>
Ainsi entre les fuites et les duplications, les performances mesurées sur l’échantillon de test sont faussées d’au moins 4,015%.<br>

-	Configuration à 4 entités (LOC, ORG, PER et MISC) :<br>
o	Fuites entre l'échantillon d'entraînement et l'échantillon de test : 1028 lignes soit 0,384 %<br>
o	Fuites entre l'échantillon de validation et l'échantillon de test : 134 lignes soit 0,552 %<br>
o	Lignes dupliquées dans l'échantillon d'entraînement : 1779 lignes soit 0,664 %<br>
o	Lignes dupliquées dans l'échantillon de validation : 1 lignes soit 0,004 %<br>
...
Notons également qu’en plus du choix du modèle, la tâche de NER peut s’effectuer au niveau du <i>token</i> ou bien au niveau d’un <i>n</i>-gram de <i>tokens</i> consécutifs. Cette étendue de <i>tokens</i> ayant le nom de <i>span</i> dans la littérature.
Une librairie efficace et simple d’utilisation pour effectuer de la NER au niveau d’une <i>span</i> est <a href="https://github.com/tomaarsen/SpanMarkerNER">SpanMarker</a> de Tom Aarsen (2023).
...
On peut observer que les modèles NERmembert (quel que soit le nombre d’entités considérés) performent mieux que les autres modèles. Cela s’explique vraisemblablement par le fait qu’ils aient vu trois fois plus de données lors de l’entraînement.<br>
De même, on peut voir que les modèles larges ont de meilleurs résultats que les modèles bases.<br>
Notons que la différence de performance est particulièrement marquée entre les NERmembert 3 entités et les modèles non NERmembert avec des écarts de plus de 20 points sur l’entité ORG par exemple.<br>
Les NERmembert apparaissent comme plus généralistes comparés au non NERmembert qui ont été entraîné uniquement sur le jeu de données WikiNER et donc spécialisés sur des données de type Wikipédia. Ils obtiennent d’ailleurs de meilleurs résultats que les NERmembert sur ce jeu de données-là.
Nous constatons aussi qu’alors que <a href="https://hf.co/cmarkea/distilcamembert-base-ner">cmarkea/distilcamembert-base-ner</a> annonce les meilleurs résultats sur WikiNER, une fois les fuites et duplications du jeu de données supprimées <a href="https://hf.co/Jean-Baptiste/camembert-ner">Jean-Baptiste/camembert-ner</a> donne en réalité les meilleurs résultats.
Une source d’étonnement pour nous cependant, est que nous nous étions attendus à de bonnes performances de la part de ces deux modèles le jeu de données WikiAnn qui est aussi issu de Wikipédia (les textes en entrée étant plus cours que ceux de WikiNER), ce que nous n’observons pas dans les résultats.<br>
Enfin, on peut remarquer un écart sur entre les modèles NERmembert 3 entités et 4 entités. La différence entre les deux configurations est que les NERmembert 3 entités ont vu un peu plus de données, à savoir le jeu de données WikiAnn qui fait environ 25 000 lignes supplémentaires. Dans le détail des résultats par jeu de données disponibles ci-dessous, on peut d’ailleurs voir que le modèle base à 4 entités donne des performances équivalentes au modèle base à 3 entités sur les jeux de données qu’ils ont en commun mais a des difficultés sur WikiAnn.<br>
...
Ci-dessous, nous listons les tableaux des résultats des performances des différents modèles considérés dans la configuration à quatre entités (PER, LOC, ORG, MISC) du jeu de données <a href="https://hf.co/datasets/CATIE-AQ/frenchNER_4entities"> frenchNER_4entities</a>.<br>
...
<br>
A nouveau, pour des raisons de place, nous ne présentons que le F1 des différents modèles. Vous pouvez consulter les résultats complets (i.e. toutes les métriques) dans les cartes de modèles disponibles sur <a href="https://hf.co/collections/CATIE-AQ/french-ner-pack-658aefafe3f7a2dcf0e4dbb4">Hugging Face</a>.<br>
...
A nouveau, les NERmembert apparaissent comme plus généralistes comparés au non NERmembert. Comme pour la configuration à trois entités, ces modèles obtiennent de meilleurs résultats sur WikiNER mais ont des difficultés sur les autres jeux de données.<br>
Il y a un écart important sur la catégorie MISC. Une explication pourrait être la nature de la définition de cette entité. En effet, pour WikiNER, il s’agit principalement de nom d’œuvre (livres ou films par exemple), là où MultiNERD et MultiCoNER gèrent en plus des noms médicaux (maladies/symptômes) et des produits (marques de véhicule/nourriture/vêtements).
...
Nous avons listé ci-dessous des modèles entraînés au niveau du <i>token</i>. Nous avons également testé d’entraîner des modèles au niveau de <i>n</i>-gram de <i>tokens</i> consécutif avec la librairie SpanMarker. La littérature sur le sujet (le <a href="https://raw.githubusercontent.com/tomaarsen/SpanMarkerNER/main/thesis.pdf">mémoire de Tom Aarsen</a> auteur de SpanMarker en fait une bonne synthèse) tend à indiquer que les modèles entraînés au niveau de <i>span</i> donnent de meilleures performances que les modèles entraînés au niveau du <i>token</i>.  
...
Nous avançons deux possibilités à ces résultats. La première est que les performances peuvent varier en fonction des langues (Tom Aarsen nous ayant indiqué que des observations similaires ont été observé en espagnol).
...
Dans la section sur la description des données, nous avons listés les effectifs disponibles par type d’entités. Il est possible de relever un déséquilibre se ressentant par la suite sur les résultats (pour les entités ORG et MISC notamment). Un travail important consisterait ainsi à équilibrer les entités à notre disposition en effectuant par exemple de l’augmentation de données par simple substitution d’une valeur d’une entité par un autre du même type. Cela peut se faire en utilisant des entités déjà présentes dans notre jeu de données (ce qui pourrait permettre de rendre la valeur d’une entité plus robuste au contexte l’entourant) ou bien issues de sources externes (pour les ORG on peut penser à des données issues des chambres de commerce ou de l’INSEE par exemple).<br> 
