...
Named Entity Recognition (often abbreviated to NER) is an NLP task consisting of labeling sequences of words in a text that are names of things (people, companies, genes, proteins, etc.).<br>
...
-	<a href="http://dx.doi.org/10.1016/j.artint.2012.03.006">WikiNER</a> by Nothman et al. (2013) which is a dataset (for the French part) of 120,682 training data and 13,410 test data. The entities announced are LOC (Location), ORG (Organizations), PER (Persons) and MISC (Miscellaneous) and are distributed as follows:<br>
<table>
<thead>
<tr>
<th>Split</th>
...
-	<a href="https://arxiv.org/abs/1902.00193">Wikiann</a> by Rahimi et al. (2019) based on Pan, Xiaoman, et al. (2019) which is a dataset with 20,000 training data, 10,000 validation data and 10,000 test data. The entities announced are LOC, ORG, PER and MISC and are distributed as follows:<br>
...
<br><br>
These are the main "clean" datasets, in the sense that they can be used easily because they are available on the Hugging Face Hub, but we'll see in the next section that the data quality of some of the datasets cited just now is questionable and therefore required cleaning before they could be used to train a model. <br><br>


It should be noted that other NER datasets exist in French, but are either of poorer quality, require a great deal of pre-processing to be usable, or are too specialized. Examples include the following datasets:
...
-	<a href="https://arxiv.org/abs/1703.00948.pdf">DAWT</a> by Spasojevic et al. (2017) no longer available online following the buyout of the company that created the dataset.<br>
...
<p style="text-align:justify;">
Looking at the quality of the datasets listed, we found that individually they contained data leaks between training and test samples, as well as duplicate data. 

It should also be noted that, once individual cleaning has been completed, additional cleaning must be carried out at the collective level. This is because data present in the training sample of an A dataset, and therefore not present in the test sample of A, may be present in the test sample of B, creating a leak when the A+B dataset is created.

By way of information, the statistics concerning the number of leaks and duplications for the five NER datasets we selected for our experiments are as follows:

-	Multiconer :
o Leakage between training split and test sample: 13 lines or 0.083%.
o Leaks between the validation split and the test split: 0 lines, i.e. 0%.
o Duplicated lines in the training split: 170 lines or 1.081% 
o Duplicated lines in the test split: 170 lines or 1.081
o Duplicated lines in the validation split: 2 lines or 0.242% 
o Duplicated lines in the test split: 2 lines or 0.233%.
Thus, between leaks and duplications, the performance measured on the test split is biased by at least 1.750%.


-	Multinerd :
o Leakage between training and test splits: 69 lines or 0.049%.
o Leaks between validation and test splits: 20 lines or 0.114%.
o Duplicated lines in the training split: 2,600 lines (1.846%)
o Duplicated lines in the validation split: 58 lines or 0.329%.
o Duplicated lines in the test split: 58 lines or 0.328%.
Thus, between leaks and duplications, the performance measured on the test split is biased by at least 0.831%.
...
-	Wikiner :
o Leakage between training and test splits: 22 lines or 0.019%.
o Leakage between validation and test splits: 20 lines or 0.331%.
o Duplicated lines in the training split: 562 lines (0.49%)
o Duplicated lines in the validation split: 17 lines or 0.282%.
o Duplicated lines in the test split: 17 lines or 0.127%.
Thus, between leaks and duplications, the performance measured on the test split is biased by at least 0.440%.



-	Wikiann :
o Leakage between training and test splits: 742 lines or 3.71%.
o Leakage between validation split and test split: 473 lines or 4.73%.
o Duplicated lines in the training split: 1889 lines (9.445%)
o Duplicated lines in the validation split: 644 lines (6.44%)
o Duplicated lines in the test split: 644 lines or 6.44%.
...
o Leakage between training and test splits: 1071 lines or 0.371% o Leakage between validation and test splits: 613 lines or 1.799
o Leakage between validation split and test split: 613 lines (1.799%)
o Duplicated lines in the training split: 1839 lines or 0.638% o Duplicated lines in the test split: 1839 lines or 0.638
o Duplicated lines in the validation split: 8 lines or 0.023%.
o Duplicated lines in the test split: 8 lines or 0.019%.
...
o Leakage between training split and test split: 1028 lines or 0.384%.
o Leaks between the validation split and the test split: 134 lines or 0.552%.
o Duplicated lines in the training split: 1779 lines (0.664%)
o Duplicated lines in the validation split: 1 lines or 0.004%.
...
In practice, encoder models are the most widely used. Because they are best suited to solving classification tasks, and probably out of habit. Indeed, in the case of French, encoder transformers were available before decoder transformers and full transformers.
Let us also point out that the CamemBERT model by <a href="https://arxiv.org/abs/1911.03894">Martin et al. (2019)</a> seems to be used more than the FlauBERT by <a href="https://arxiv.org/abs/1912.05372">He et al. (2019)</a> for the NER task, without explanation why. <br>
...
An efficient and easy-to-use library for performing NER at the span level is 
<a href="https://github.com/tomaarsen/SpanMarkerNER">SpanMarker</a> by Tom Aarsen (2023).
...
The limitation of these models is that none of them uses all the data available, since all three are trained only on the WikiNER dataset. 
...
It should also be noted that no French NER model is available in large size.<br>


Given these limitations, we have developed our own model at CATIE: NERmembert. This uses all the quality data available in open-source and has been trained in two configurations (on 3 entities or 4 entities) and is available for each of them in basic and large versions. All free and open-source:<br>
...
Note that the difference in performance is particularly marked between NERmembert 3 entities and non-NERmembert models, with differences of over 20 points on the ORG entity, for example.<br>
...
<br>
Again, for reasons of space, we only show the F1 of the various models. You can consult the complete results (i.e. all metrics) in the model cards available at <a href="https://hf.co/collections/CATIE-AQ/french-ner-pack-658aefafe3f7a2dcf0e4dbb4">Hugging Face</a>.<br>
